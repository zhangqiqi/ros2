 ...
 * @brief 同级指令列表
 */
SIMPLEQ_HEAD(SSNP_SHELL_LIST, SSNP_SHELL_CMD);

/**
 * @brief shell指令基本节点类型
 */
struct SSNP_SHELL_CMD {
	char cmd[16];      /**< 单指令字符串 */
	enum SSNP_SHELL_CMD_TYPE type;      /**< 单指令类型 */

	void *cb_handle;      /**< 指令处理回调操作句柄 */
	SSNP_SHELL_PROC_CB proc_cb;      /**< 指令处理回调 */

	struct SSNP_SHELL_LIST sub_cmds;      /**< 下级指令列表 */

	SIMPLEQ_ENTRY(SSNP_SHELL_CMD) SHELL;
};

static struct SSNP_SHELL_LIST __snp_shell = {0};
static struct SSNP_SHELL_LIST *snp_shell = &__snp_shell;      /**< shell根节点 */

static struct SSNP_SHELL_CMD shell_cmd_init_arr[32] = {0};      /**< 预分配32个节点使用 */

/**
 * @brief 初始化协议栈shell模块
 */
void ssnp_shell_init(void)
{
 ...
 */
static struct SSNP_SHELL_CMD *__snp_shell_get_new_cmd_node(void)
{
 ...
 * @return 0 安装成功 其它 失败
 */
int32_t ssnp_shell_setup(char **descriptor, int32_t num, SSNP_SHELL_PROC_CB proc_cb, void *cb_handle)
{
	struct SSNP_SHELL_LIST *_var_cmd_list = snp_shell;
	struct SSNP_SHELL_CMD *_var_cmd;
	int i = 0;
	for (i = 0; i < num; i++)
	{
 ...
 */
int32_t ssnp_shell_exec(char *cmd_str, char *res_str, int32_t res_size)
{
	struct SSNP_SHELL_LIST *_var_cmd_list = snp_shell;
	struct SSNP_SHELL_CMD *_var_cmd = NULL;
	struct SSNP_SHELL_CMD *_var_placeholder_cmd = NULL;
